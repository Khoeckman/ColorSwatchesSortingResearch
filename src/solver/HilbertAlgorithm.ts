// @source https://github.com/MrDesjardins/hilbert-curve-ts/blob/master/src/hilbertAlgorithm.ts
 ...
 */
export default class HilbertAlgorithm {
  /**
  ...
  public indexToPoint(index: number): CoordinateValue {
    ...
    let rx: PointValue
    let ry: PointValue
    let orderIndex: number = 1 // Always start at the first order and move up
    ...
    while (orderIndex < numberRow) {
      ...
      orderIndex = orderIndex * 2 // Each order double the size of element per row (and column)
    }
    return point
  }
   ...
   */
  public static movePoint(point: CoordinateValue, rx: PointValue, ry: PointValue, orderIndex: number): void {
   ...
   *
   * Example: pointToIndex(indexToPoint(123)) returns 123
   */
  public pointToIndex(point: CoordinateValue): number {
    const numberOfRow: number = 2 ** this.order
    ...
    let ry: PointValue
    let index: number = 0
    for (let rowIndex: number = numberOfRow / 2; rowIndex > 0; rowIndex = Math.floor(rowIndex / 2)) {
   ...
   */
  public static getNewIndexFromRows(rowsIndex: number, rx: PointValue, ry: PointValue): number {
   ...
   */
  public static getRxFromPoint(point: CoordinateValue, orderIndex: number): PointValue {
   ...
   */
  public static getRyFromPoint(point: CoordinateValue, orderIndex: number): PointValue {
   ...
   */
  public static getPointValueFromNumber(numberN: number, orderIndex: number): PointValue {
  ...
  public offsetPoint(point: Readonly<CoordinateValue>, projectionWidth: number): PointWithPixelValue {
  ...
  public deoffsetPoint(point: Readonly<PointWithPixelValue>, projectionWidth: number): CoordinateValue {
    if (projectionWidth < 0) {
      throw new Error('Projecting a point must be on a positive width')
    }
    const numberOfRows = Math.pow(2, this.order)
